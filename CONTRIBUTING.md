...
- Для отладки 1С: ничего ставить не нужно, расширение скачивает адаптер с рантаймом внутри; на Linux рантайму нужен системный пакет `libicu`.
...
- Свой канал Output заводите только под живой поток чужого долгоживущего процесса, который читают как консоль (так сделано у автономного сервера с выводом `ibsrv`). Всё, что пишет сам код расширения, идёт в общий журнал `1C: Platform Tools` со скоупом модуля: один канал с уровнями и фильтром удобнее набора мелких, а событие не дублируется в двух местах.
...
- Рутинное подтверждение («дерево обновлено», «файл создан», «свойства сохранены») показывайте через `notifyQuiet` из `shared/notify`: строка состояния и запись в журнал. Всплывающее окно оставляйте для ошибок, предупреждений, сообщений с кнопками и подсказок, что делать дальше.
...
|-----------------------|---------------------------------------------------------------------------|---------------------------|-------------------------------------------------------------------------------------------------------------------------|
| Адаптер отладки (DAP) | [onec-debug-adapter](https://github.com/yellow-hammer/onec-debug-adapter) | `dotnet build -c Release` | `1c-platform-tools.components.path.adapter` — путь к `OnecDebugAdapter.dll`, `components.autoload.adapter: false`         |
| Дерево метаданных     | [md-sparrow](https://github.com/yellow-hammer/md-sparrow)                 | `gradlew shadowJar`       | `1c-platform-tools.components.path.metadataJar` — путь к `md-sparrow-*-all.jar`, `components.autoload.metadataJar: false` |
| Менеджер версий OneScript | [ovm](https://github.com/oscript-library/ovm)                         | не требуется              | `1c-platform-tools.components.path.ovm` — путь к `ovm.exe`, `components.autoload.ovm: false`                              |
| Отчёты Allure         | [allure2](https://github.com/allure-framework/allure2)                    | не требуется              | `1c-platform-tools.components.path.allure` — путь к `allure`, `components.autoload.allure: false`                                   |

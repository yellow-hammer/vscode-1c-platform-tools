...
async function createProfile(vrunner: VRunnerManager, refresh: () => void): Promise<void> {
	...
	const fileName = `env.${id.trim()}.json`;
	await openEnvFile(vrunner, fileName);
	await vrunner.setActiveEnvProfileId(id.trim());
	refresh();
	vscode.window.showInformationMessage(`Профиль «${id.trim()}» создан и выбран активным.`);
}
...
async function selectProfile(vrunner: VRunnerManager, refresh: () => void): Promise<void> {
	...
	for (;;) {
		...
		items.push(
			...
			{ label: '$(go-to-file) Открыть файл активного профиля', action: 'open' }
		);

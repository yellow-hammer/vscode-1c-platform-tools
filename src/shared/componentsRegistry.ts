 ...
 *
 * Компоненты загружаются из релизов GitHub в кэш расширения. Обычная работа
 * идёт через `ensure*`: они уважают настройку своего пути и выключенную
 * автозагрузку. Команда обновления работает иначе - человек нажал «обновить»,
 * значит нужна свежая загрузка, поэтому используется `download`.
 *

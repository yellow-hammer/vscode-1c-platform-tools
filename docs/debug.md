...
- [Отладка 1С](#отладка-1с)
  ...
  - [Настройка](#настройка)
    - [1. Строка подключения — из активного профиля запуска](#1-строка-подключения--из-активного-профиля-запуска)
...
- Исходный код конфигурации в каталоге из настройки `path.cf` (по умолчанию `src/cf`): формат конфигуратора или формат EDT.
...
Расширение сгенерирует конфигурацию по настройкам путей проекта (`1c-platform-tools.path.*`):
...
| `extensions`                         | каталоги исходников расширений (по одному на расширение): и решения из `path.cfe`, и тестовых из `<path.tests>/cfe` - иначе в тест не зайти точкой останова                                                                                                                                                                           |
...
Строка подключения в launch.json не указывается — она всегда берётся из активного профиля запуска.
...
2. Собранный файл — в каталоге из `externalFilesBuilds`; имя файла должно совпадать с именем обработки или отчёта (`МояОбработка.xml` или `МояОбработка.mdo` → `build/out/epf/МояОбработка.epf`). Собрать можно командой **Собрать внешнюю обработку** / **Собрать внешний отчёт** (дерево **1С: Инструменты** → **Внешние файлы**).
...
Скрыть всё — команда **1С: Отладка: Скрыть результаты замера производительности**. Замер автоматически выключается при завершении сессии — «забытый» замер не останется висеть на сервере отладки.

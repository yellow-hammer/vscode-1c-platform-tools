...
2. Собранный файл — в каталоге из `externalFilesBuilds`; имя файла должно совпадать с именем обработки или отчёта (`МояОбработка.xml` или `МояОбработка.mdo` → `build/out/epf/МояОбработка.epf`). Собрать можно командой **Собрать внешнюю обработку** / **Собрать внешний отчет** (дерево **Инструменты 1С** → **Внешние файлы**).

...
Поддержка поставщика читается из правил поставки в выгрузке. Объект, запрещённый к изменению, помечен в дереве серым замочком на пиктограмме; замочек стоит и на самой конфигурации, и на разделах и узлах состава запрещённого объекта, включая листья внутри подсистем. Объект с разрешённым изменением дерево не подкрашивает, режим виден в подсказке узла. Запись в запрещённый объект не выполняется: md-sparrow отказывает до правки файлов, и выгрузка остаётся нетронутой.

Поддержкой можно управлять из дерева. Пункт **Включить возможность изменения** на конфигурации открывает правила поставщика: каждому объекту ставится «не редактируется», как это делает конфигуратор. Пункт **Режим поддержки** на объекте предлагает разрешить изменение, запретить его или снять объект с поддержки. Меняются только цифры режимов в файле правил, всё остальное содержимое остаётся байт-в-байт прежним.

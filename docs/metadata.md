...
В контекстном меню формы в дереве метаданных есть пункт **«Открыть форму»**. Вкладка показывает форму так, как её видит конфигуратор: дерево элементов, схематичное превью, данные формы (реквизиты, команды, параметры, обработчики) и свойства выделенного элемента.

...
| Что выделено | Какие свойства |
| --- | --- |
| Элемент формы на превью | Весь состав свойств вида элемента из модели формата |
| Объект метаданных в дереве | Те же свойства, что на вкладках панели объекта |
| Конфигурация или расширение | Свойства `Configuration.xml` |
| Внешний отчёт или обработка | Имя, синоним, комментарий |
| Реквизит, табличная часть, значение, измерение, ресурс | Имя, синоним, комментарий, тип |
| Форма, команда, макет | Имя: своих свойств у них в файле объекта нет |

Выделили объект в дереве - палитра показала его свойства. Пока панель закрыта, ничего не читается и не всплывает; фокус она не забирает. Свойства выделенного элемента формы выделение в дереве не подменяет, пока панель не открыта.
...
Значения правятся прямо в строках: поле для строк и чисел, флажок для булевых, выпадающий список для перечислений. Изменённые строки выделяются, а записываются все разом по кнопке **«Записать»** внизу панели. Там, где править нечего, кнопки нет.

- **Элемент формы.** После записи форма перечитывается, вкладка с превью обновляется. Значение, совпавшее со значением по умолчанию, убирается из файла - платформа его туда и не пишет. Правится только сам узел свойства, остальной файл остаётся как был. Имя и идентификатор элемента не меняются: платформа держит их атрибутами.
- **Объект, конфигурация, расширение, внешний артефакт.** Правки уходят той же операцией записи, что и из панели-вкладки.
- **Узлы состава.** Правятся синоним и комментарий. Имя не правится: переименование задевает ссылки, это отдельная операция дерева. Тип показывается строкой.

Составные редакторы - список ссылок, подбор типа, ссылка на модуль, шрифт и цвет - показываются текстом: им нужна ширина панели-вкладки, там они и правятся. Палитра её не заменяет, а дополняет компактным видом.

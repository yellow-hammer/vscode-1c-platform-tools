...
У конфигурации и расширения перечислимые свойства подписаны словарём формата, назначения использования отмечаются флажками, основные роли ведутся плашками: крестик убирает роль, «+ Роль…» подбирает из ролей конфигурации. Свойства панель берёт из формата выгрузки, поэтому у объектов одного рода набор одинаковый: поле ввода, полнотекстовый поиск, история данных, поля блокировки, вспомогательные формы, ввод на основании. Основанием ввода бывает не только справочник или документ - панель предлагает всё, что разрешает платформа.
...
У формы файлов два, и они разведены: **Открыть XML** показывает описание формы (`<Объект>/Forms/<Форма>.xml` - имя, синоним, тип), **Открыть XML содержимого** - саму форму с элементами и реквизитами (`Ext/Form.xml`). У общей формы описание лежит в её собственном XML, а содержимое - в `<Форма>/Ext/Form.xml`.

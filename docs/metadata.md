...
Поддержкой управляют в той же последовательности, что в конфигураторе. Сначала пункт **Включить возможность изменения** на конфигурации: правило поддержки объектов по умолчанию выбирается из двух вариантов - «не редактируется» либо «редактируется с сохранением поддержки». После этого на объекте появляется пункт **Режим поддержки**: разрешить изменение, запретить его или снять объект с поддержки; пока правила закрыты, пункта нет, как и в конфигураторе. Меняются только цифры режимов в файле правил, всё остальное содержимое остаётся байт-в-байт прежним.

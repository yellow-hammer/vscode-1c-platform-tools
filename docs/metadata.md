...
Пункт **Свойства** в контекстном меню открывает панель со свойствами, разложенными по вкладкам. Пункт один на все узлы: у конфигурации и расширения открываются их свойства, у внешнего отчёта и обработки - свои, у общей формы - сама форма, а у узла состава свойства показывает палитра. Повторный вызов не открывает вторую вкладку: она ищется по объекту, поэтому соседний объект получает свою.
...
Списки реквизитов, значений, измерений, ресурсов и команд ведутся одинаково: добавление, переименование, удаление, изменение порядка, правка синонима и типа. Те же операции есть и в дереве: **Добавить** стоит на разделе состава, а **Переименовать**, **Дублировать** и **Удалить** - на самом узле. У команды объекта дублирования нет: его не умеет md-sparrow. У команды вместе с ней переезжает и удаляется каталог её модуля. Новый элемент создаётся со строковым типом длины 10 — платформа требует тип всегда, поэтому пустым он не остаётся.
...
Свойства живут в отдельной панели **«1С: Свойства»** со своим значком на панели действий; открывает её пользователь, сама она не всплывает. Если значка не видно, панель открывает команда **«1С: Метаданные: Открыть панель свойств»**. Панель можно перетащить куда удобно, в том числе в правую боковую, - тогда форма и свойства видны одновременно.

...
Работа с поддержкой включается настройкой `metadata.supportEnabled`, по умолчанию она выключена: выгрузка правится так, будто поставки нет, признаков в дереве не появляется, отказов в правке тоже.

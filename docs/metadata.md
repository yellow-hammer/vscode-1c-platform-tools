...
Пункт **Свойства** в контекстном меню открывает вкладку со свойствами, разложенными по группам. Пункт один на все узлы и вкладка одна: конфигурация, расширение, объект, внешний отчёт и обработка открываются одинаково. Вид без развёрнутой спеки получает ту же форму: «Основные» с именем, синонимом, комментарием и скалярными свойствами вида плюс свои разделы состава; раздел, которого у вида не бывает, на панель не попадает. Флажки, числа, перечислимые значения и ссылки правятся своими контролами. У объекта конфигурации есть вкладка «Подсистемы» с деревом и флажками участия: изменения пишутся в состав подсистем. У подсистемы есть вкладка «Командный интерфейс», её же открывает одноимённый пункт контекстного меню. Список команд собирается из состава подсистемы: стандартные команды объектов видны и без файла настроек. Флажок пишет общую видимость команды в файл настроек, причём файл получает только записанные раньше и изменённые сейчас команды. Ниже остальные секции файла: у размещения группа панели меняется выпадающим списком, а порядок команд, подсистем и групп переставляется стрелками теми же, что в списках состава. Команды подписаны как везде - «Справочник: Товары: Открыть список», панели навигации и действий тоже по-русски. У роли есть вкладка «Права»: кросс-таблица, строки - объекты конфигурации, колонки - права, на пересечении флажки. По умолчанию видны только объекты с выданными правами, флажок «Все объекты» показывает всю конфигурацию; список сужают фильтр по имени и отбор по виду. Флаги прав по умолчанию в шапке тоже правятся флажками. Выданное право тянет зависимые, как в конфигураторе: изменение и удаление требуют чтения, проведение - изменения, интерактивные - своих базовых; снятие базового снимает зависимые. Право пишется записью в файл прав роли, снятое убирается, опустевший объект уходит из файла целиком. Состав правится деревом с флажками на вкладке «Состав», внизу - плоский список вошедшего: у подсистемы и функциональной опции весь состав конфигурации, у параметра опции - использование, у последовательности - входящие документы и движения регистров, у общего реквизита - объекты с режимом использования, у плана обмена - объекты с авторегистрацией. Состав критерия отбора - ссылки на реквизиты: они снимаются, добавление пока в дереве кандидатов не предлагается. Заимствованный объект расширения показывает ту же форму без правки. У узла состава свойства показывает палитра. Повторный вызов не открывает вторую вкладку: она ищется по объекту, поэтому соседний объект получает свою.

У конфигурации и расширения перечислимые свойства подписаны словарём формата, назначения использования отмечаются флажками, основные роли ведутся плашками: крестик убирает роль, «+ Роль…» подбирает из ролей конфигурации. Происхождение объекта видно в шапке панели: бейдж «Заимствованный» у объекта расширения, «На поддержке» с поставщиком и версией в подсказке - у объекта поставщика. Объект, который менять нельзя, открывается только на просмотр: поля недоступны, кнопки записи нет, а причина написана строкой над вкладками. Так же ведут себя свойства конфигурации на полной поддержке, просмотр её формы и палитра свойств.

Запрет действует и в дереве, но пункты меню остаются на месте: контекстное меню VS Code не умеет показывать пункт недоступным, поэтому вызов правки у объекта под поддержкой отвечает подсказкой сменить режим поддержки или снять конфигурацию с поддержки. Так же отвечает добавление новых объектов в конфигурацию на полной поддержке.

Своё правило поддержки есть не только у объекта: форма, макет, реквизит, табличная часть, измерение, ресурс, значение перечисления и команда - каждый отдельный субъект правил, и в дереве все они несут собственный замок. Пункт **Режим поддержки** меняет режим именно того узла, на котором вызван, а на объекте спрашивает, применить правило только к объекту или ко всему, что в нём. Правка запрещённого элемента отклоняется, даже когда сам объект открыт: переименование и удаление реквизита проверяются по его правилу, удаление формы - по правилу формы.
...
Раскладка вкладок одна на все виды и повторяет конфигуратор: **Основные**, **Данные**, вкладка сути вида (адресация, компоновка, субконто, расчёт, движения, регистрируемые документы, обмен данными), **Формы**, **Команды**, **Макеты**, **Ввод на основании**. Пустые вкладки не показываются. Состав объекта - слева на той вкладке, к которой относится: реквизиты и табличные части на «Данных», команды на «Командах», макеты на «Макетах». Форма на вкладке «Формы» открывается щелчком по имени, крестик удаляет её вместе с файлами, кнопка «+ Форма…» создаёт пустую управляемую форму из эталона платформы. Те же «Добавить» и «Удалить» есть в дереве: на разделе «Формы» и на самой форме. Пункт **Добавить объект в расширение** на объекте создаёт заимствованную копию в выбранном расширении: платформа свяжет её с оригиналом по имени. Там же, на «Данных», поиск по свойствам.

Списки реквизитов, значений, измерений, ресурсов, команд и признаков учёта ведутся одинаково: добавление, переименование, удаление, изменение порядка, правка синонима и типа. Те же операции есть и в дереве: **Добавить** стоит на разделе состава, а **Переименовать**, **Дублировать** и **Удалить** - на самом узле. У команды объекта дублирования нет: его не умеет md-sparrow. У команды вместе с ней переезжает и удаляется каталог её модуля. Новый элемент создаётся со строковым типом длины 10 — платформа требует тип всегда, поэтому пустым он не остаётся.

Изменения копятся в панели и уходят в файлы одним нажатием **Сохранить**. Заимствованные объекты расширений открываются только на просмотр: у них свои правила состава XML. В дереве они помечены значком **А** на пиктограмме.

Поддержка поставщика читается из правил поставки в выгрузке. Объект, запрещённый к изменению, помечен в дереве жёлтым замочком на пиктограмме; замочек стоит и на самой конфигурации, и на разделах и узлах состава запрещённого объекта, включая листья внутри подсистем. Объект с разрешённым изменением дерево не подкрашивает, режим виден в подсказке узла. Запись в запрещённый объект не выполняется: md-sparrow отказывает до правки файлов, и выгрузка остаётся нетронутой.

Поддержкой управляют в той же последовательности, что в конфигураторе. Пункт **Включить возможность изменения** на конфигурации открывает только корень - свойства конфигурации и добавление объектов; сами объекты остаются «не редактируется», и на каждом появляется пункт **Режим поддержки**: разрешить изменение, запретить его или снять объект с поддержки. Пока правила закрыты, пункта на объекте нет. Меняются только цифры режимов в файле правил, всё остальное содержимое остаётся байт-в-байт прежним. Правка идёт поверх того снимка правил, который показан в дереве: если файл успел измениться конфигуратором или соседней сессией, запись отклоняется с просьбой обновить дерево. Пункт **Снять с поддержки** на конфигурации удаляет правила вместе с поставками поставщика из `Ext/ParentConfigurations`: правил больше нет, вся выгрузка редактируется свободно, автоматическое обновление от поставщика становится недоступно. Пока каталог поставок заведён, но пуст, правила не правятся: без файла поставки следующая выгрузка платформы потеряет её.

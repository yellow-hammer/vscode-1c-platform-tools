...
Панель состоит из плашек **Поиск**, **Фильтры**, **Дерево метаданных**, **Свойства** и **Помощь и поддержка**. По умолчанию развёрнуто только дерево.
...
Клик по форме в дереве метаданных открывает её вкладкой: дерево элементов, схематичное превью и данные формы (реквизиты, команды, параметры, обработчики). Свойства выделенного элемента показывает плашка **«Свойства»**.
...
Плашка **«Свойства»** показывает свойства того, что выделено, группами - как палитра свойств конфигуратора. Сейчас её наполняет редактор формы; дальше через неё пойдут свойства объектов метаданных и их дочерних узлов, поэтому содержимое описывается данными: источник отдаёт группы и строки, плашка их рисует.

Плашку можно перетащить в правую боковую панель - тогда свойства и форма видны одновременно.

...
**Общий запуск идёт одним процессом.** Кнопка «Запустить все» и запуск группы выполняются одним прогоном там, где фреймворк умеет выполнять набор целиком: для 1С это один сеанс вместо сеанса на каждый файл. Результаты общего отчёта расходятся по файлам дерева автоматически. Если фреймворк набор целиком не выполняет, файлы прогоняются поштучно.
...
- `src/tests/` — разобранные исходники тестовых обработок 1С, по подкаталогу на обработку. Панель тестирования показывает методы прямо из исходника и собирает обработку перед прогоном (неизменённые обработки не пересобираются);
- `build/out/tests/` — собранные из исходников `.epf`: артефакт сборки, в git не попадает.

## Команды сборки unit тестов

- **Собрать unit тесты** — собирает тестовые обработки из исходников `src/tests` в `build/out/tests`.
- **Разобрать unit тесты** — раскладывает хранимые `.epf` в исходники: удобно для первичного переноса тестов под контроль версий.

Обе команды доступны в дереве **Инструменты 1С** (группа «Внешние файлы», рядом со сборкой обработок и отчётов) и в палитре команд (F1 → «1С: Собрать unit тесты»). Отдельно вызывать сборку не требуется: панель тестирования собирает обработку перед прогоном сама.
...
- **Allure отчёт** собирает результаты всех фреймворков: команда сканирует `build/out` и берёт все существующие источники — allure-каталоги (smoke, syntax-check, VA), jUnit (yaxunit, onescript, 1bdd) и Cucumber JSON (VA); дубли одного прогона в двух форматах исключаются. Командная строка Allure загружается с GitHub Releases при первом отчёте и обновляется автоматически, как остальные внешние компоненты; своя установка указывается настройкой `1c-platform-tools.allure.path`, автозагрузка выключается настройкой `1c-platform-tools.components.allureAutoload`. Для работы Allure нужна установленная Java.

...
| **OneScript**           | `.os`-тесты (`ИсполняемыеСценарии` или аннотации `&Тест` / `&ПараметризованныйТест`) | `1testrunner -run ... xddReportPath` / `oneunit execute --junit` | jUnit в `build/out/onescript`                  |
...
- Источник — jUnit-отчёт прогона; путь берётся из env-файла **активного профиля запуска** (`env.json` или `env.<id>.json`), секция `syntax-check`, опция `--junitpath` (формат vanessa-runner v2), с откатом на `build/out/syntax-check/junit/junit.xml`.

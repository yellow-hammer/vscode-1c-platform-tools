...
Настраивается только корень: `1c-platform-tools.path.tests`. Раскладка внутри фиксированная -
...
| `path.tests`                                       | `tests`                        | Корень тестов: `*.os`, `cfe/`, `epf/`             |
| `test.featuresPath`                              | `features`                     | Сценарии Gherkin                                  |
| `test.onescriptTestsPath`                        | `tests`                        | Устарела: каталог тестов задаётся в `path.tests`  |
...
`path.cfe` и тестовые расширения не трогают. Это та же граница, что у тестовых обработок -

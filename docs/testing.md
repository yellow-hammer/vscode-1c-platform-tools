...
Настроек на эти каталоги нет: тестовые расширения и обработки находятся по маркерам, а от
поставляемых их отличает каталог `tests` в пути, см. [Раскладка проекта](tools.md#раскладка-проекта).
Каталог скриптовых тестов `*.os` задаётся настройкой `test.path.onescriptTests`: у них маркеров нет.
...
| `test.path.onescriptTests`                       | `tests`                        | Каталог скриптовых тестов `*.os`                  |
...
Почему отдельно: команды группы «Расширения» работают с расширениями решения, а тестовые идут
своими командами. Это та же граница, что у тестовых обработок - исходники в `tests/epf`, сборка
в `build/out/tests/epf`, в поставку не входят. В дереве метаданных тестовые расширения видны
...
выбора не показывалось, задайте список в `1c-platform-tools.test.cfe.selected`: это тот же

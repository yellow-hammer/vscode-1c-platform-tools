...
тестовыми их делает `tests` в пути, см. [Раскладка проекта](tools.md#раскладка-проекта). Каталог
скриптовых тестов `*.os` задаётся настройкой `test.path.onescriptTests`.

Если исходники тестовых обработок лежат в `src/tests`, перенесите каталог в `tests/epf`. Расширение
напомнит об этом при открытии проекта и при запуске команд сборки.

...
Обе команды доступны в дереве **Инструменты 1С** (группа «Тестовое окружение», рядом с командами тестовых расширений) и в палитре команд (F1 → «1С: Собрать unit тесты»). Отдельно вызывать сборку не требуется: панель тестирования собирает обработку перед прогоном сама.

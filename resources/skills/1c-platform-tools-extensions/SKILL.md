...
В проекте может быть несколько расширений (каталоги в `src/cfe`). Если пользователь назвал конкретные — вызови команду расширения (Execute Command) с аргументом `{ "extensions": ["Имя1", "Имя2"] }`: она выполнится только для них и без окна выбора. Без параметра используется сохранённый выбор проекта (или все расширения); явный список сохранённый выбор не меняет. В схеме MCP параметра `extensions` нет — используй Execute Command.
...
- Загрузить только одно расширение: Execute Command `1c-platform-tools.extensions.loadFromSrc` с `{ "extensions": ["МоёРасширение"] }`.

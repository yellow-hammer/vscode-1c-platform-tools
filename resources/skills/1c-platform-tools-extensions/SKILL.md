...
В проекте может быть несколько расширений (каталоги в `src/cfe`). Если пользователь назвал конкретные — передай их списком в параметре `extensions` (имена каталогов), команда выполнится только для них и без окна выбора. Без параметра используется сохранённый выбор проекта (или все расширения). Явный список не меняет сохранённый выбор.

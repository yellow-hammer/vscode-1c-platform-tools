...
### Панель «Метаданные 1С» и md-sparrow

В отдельной активности на панели действий **«Метаданные 1С»** вид **«Дерево метаданных»** работает через [md-sparrow](https://github.com/yellow-hammer/md-sparrow) и показывает основную конфигурацию, расширения и внешние артефакты.

Возможности панели:

- построение и обновление дерева метаданных из выгрузки;
- создание новых объектов метаданных (включая справочники и другие типы);
- операции над дочерними узлами объекта: добавить, переименовать, удалить, дублировать (реквизиты, табличные части, реквизиты табличных частей);
- редактирование свойств поддерживаемых объектов;
- сброс кэша скачанного JAR/JRE md-sparrow.

### Диаграммы связей метаданных (ER)

Для выбранного объекта метаданных можно открыть интерактивную ER-диаграмму связей:

- построение графа по команде в контекстном меню;
- режимы охвата: выбранные объекты и прямые связи (hops), с автоматическим ограничением слишком больших графов;
- фильтры по видам связей с группировкой и счётчиками;
- поиск и добавление объектов в конструкторе диаграммы;
- экспорт схемы в Mermaid, Draw.io, SVG и PNG.
...
2. Установите расширение [1C: Platform Tools MCP](https://github.com/yellow-hammer/mcp-1c-platform-tools) (из Marketplace или вручную)

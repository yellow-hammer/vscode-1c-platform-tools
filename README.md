...
- **[Управление кластерами 1С](docs/clusters.md)** — консоль администрирования кластера серверов через `rac`: несколько подключений к `ras`, дерево баз, серверов, процессов, сеансов и соединений, таблица активности с метриками потребления, карточка свойств базы, блокировки и завершение сеансов.
...
Расширение активируется для проекта 1С при наличии файла `packagedef` в корне рабочей папки. Панели **Проекты 1С** и **Управление кластерами 1С** работают и без открытого проекта: первая поможет найти и открыть нужный, вторая не привязана к рабочей области вовсе.

## Быстрый старт

Для команд 1С нужна платформа 1С:Предприятие, OneScript, OPM и vanessa-runner — недостающее можно установить из раздела **Зависимости**.

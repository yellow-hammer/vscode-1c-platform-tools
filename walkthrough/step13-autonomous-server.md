...
**Запуск.** Команда **1С: Автономный сервер: Запустить** или клик по индикатору в статус-баре (запущенный сервер выделяется янтарным фоном). Путь к ИБ берётся из `--ibconnection` активного [профиля запуска](command:1c-platform-tools.env.selectProfile) (только файловая ИБ `/F`), параметры сервера — из настроек `1c-platform-tools.server.*`.
